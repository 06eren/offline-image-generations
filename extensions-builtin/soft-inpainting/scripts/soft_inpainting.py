    ...
    def ui(self, is_img2img):
        ...
        with InputAccordion(False, label=enabled_ui_label, elem_id=enabled_el_id) as soft_inpainting_enabled:
            with gr.Group():
                gr.Markdown(
                    """
                    Yumuşak tamamlama (Soft inpainting), maske saydamlığına göre **orijinal içerik ile tamamlanan içeriği sorunsuz bir şekilde harmanlamanızı** sağlar.
                    **Yüksek _Maske Bulanıklığı_ (Mask blur)** değerleri kullanmanız önerilir!
                    """)
                ...
                with gr.Accordion("Yardım", open=False):
                    gr.Markdown(
                        f"""
                        ### {ui_labels.mask_blend_power}

                        Orijinal içeriğin harmanlama gücü, her adımdaki (sigma'lar) azalan gürültü seviyesi değerleriyle orantılı olarak ölçeklenir.
                        Bu, gürültü giderici etkisi ile orijinal içerik koruması arasındaki dengenin her adımda yaklaşık olarak eşit olmasını sağlar.
                        Bu denge, bu parametre kullanılarak kaydırılabilir ve daha önceki veya daha sonraki adımlarda korumanın daha güçlü olup olmayacağı kontrol edilebilir.

                        - **1'in altında**: Sonlara doğru daha güçlü koruma (düşük sigma ile)
                        - **1**: Dengeli (sigma ile orantılı)
                        - **1'in üzerinde**: Başlangıçta daha güçlü koruma (yüksek sigma ile)
                        """)
                    gr.Markdown(
                        f"""
                        ### {ui_labels.mask_blend_scale}

                        Kısmen maskelenmiş görsel bölgelerinin orijinal içeriği koruma olasılığını mı yoksa tamamlanmış içeriği mi tercih edeceğini eğer.
                        Bunun {ui_labels.mask_blend_power}, CFG Ölçeği, istem ve Gürültü giderme gücüne bağlı olarak ayarlanması gerekebilir.

                        - **Düşük değerler**: Oluşturulan içeriği tercih eder.
                        - **Yüksek değerler**: Orijinal içeriği tercih eder.
                        """)
                    gr.Markdown(
                        f"""
                        ### {ui_labels.inpaint_detail_preservation}

                        Bu parametre, orijinal latent vektörlerin ve gürültüsü giderilmiş latent vektörlerin nasıl ara değerleneceğini (interpolate) kontrol eder.
                        Daha yüksek değerlerde, elde edilen harmanlanmış vektörün büyüklüğü, iki vektörün maksimumuna daha yakın olacaktır.
                        Bu, doğrusal ara değerlemede (linear interpolation) ortaya çıkan kontrast kaybını önleyebilir.

                        - **Düşük değerler**: Daha yumuşak harmanlama, ayrıntılar kaybolabilir.
                        - **Yüksek değerler**: Daha güçlü kontrast, renkleri aşırı doygunlaştırabilir.
                        """)
                    ...
                    gr.Markdown(
                        ...
                        Maskeler, gürültü giderme işleminden sonra görüntünün bir kısmının ne kadar değiştiğine bağlı olarak oluşturulur.
                        Bu maskeler, orijinal ve son görüntüleri birbirine harmanlamak için kullanılır.
                        Fark düşükse, tamamlama işlemi (inpainting) tarafından döndürülen pikseller yerine orijinal pikseller kullanılır.
                        """)
                    ...
                    gr.Markdown(
                        ...
                        Bu parametre, maskenin bu fark hassasiyetini ne kadar etkileyeceğini kontrol eder.

                        - **0**: Maskeyi yok say, yalnızca görüntü içeriğindeki farklılıkları dikkate al.
                        - **1**: Görüntü içeriği değişikliklerine rağmen maskeyi yakından takip et.
                        """)
                    ...
                    gr.Markdown(
                        ...
                        Bu değer, orijinal piksellerin %50'den az opaklığa sahip olacağı farkı temsil eder.

                        - **Düşük değerler**: Orijinal piksellerin korunması için iki görüntü yamasının (patch) neredeyse aynı olması gerekir.
                        - **Yüksek değerler**: İki görüntü yaması çok farklı olabilir ve orijinal pikseller hala korunur.
                        """)
                    ...
                    gr.Markdown(
                        ...
                        Bu değer, orijinal ve tamamlanmış içerik (inpainted content) opaklığı arasındaki kontrastı temsil eder.

                        - **Düşük değerler**: Harmanlama daha kademeli olacak ve daha uzun geçişlere sahip olacaktır, ancak gölgelenmeye (ghosting) neden olabilir.
                        - **Yüksek değerler**: Gölgelenme (ghosting) daha az görülür, ancak geçişler çok ani olabilir.
                        """)
